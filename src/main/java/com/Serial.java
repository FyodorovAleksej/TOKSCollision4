...
public class Serial implements SerialInterface {
    ...
    public static final byte JAM = 0x79;
            ...
            while (current < bytes.length)
            {
                ...
                while (tryIteration < 5)
                {
                    ...
                    if(!isCSMAFree()) {
                        ...
                        Thread.sleep(random.nextInt((int)Math.pow(2, 5)));
                    }
                    else {
                        result = port.writeByte(bytes[current]);
                        break;
                    }
                }
                current++;
    ...
    public boolean isCSMAFree()
    {
        Long time = System.currentTimeMillis();
        return ((time / 1000) % 2 != 0);
    }
